...
const menteeSchema = Joi.object({
  ...
  mentorMujid: Joi.string().required(), // Add mentorMujid field validation
});
...
export async function POST(req) {
  try {
    ...
    try {
      ...
      return createErrorResponse("Invalid JSON input", 400, error);
    }

    const { mujid, yearOfRegistration, menteePersonalDetails, mentorMujid } = requestBody;
    ...
    return createErrorResponse("Something went wrong on the server", 500, error);
  }
}
...
export async function PUT(req) {
  try {
    ...
    try {
      ...
      return createErrorResponse("Invalid JSON input", error);
    }
    ...
    try {
      ...
      return createErrorResponse("Error updating mentee", 500, error);
    }
  } catch (error) {
    return createErrorResponse("Something went wrong on the server", 500, error);
  }
...
export async function DELETE(req) {
  try {
    ...
    return createErrorResponse("Failed to delete mentee", 500, error);
  }
}
