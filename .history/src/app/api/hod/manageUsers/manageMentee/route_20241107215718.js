...
export async function POST(req) {
  try {
    ...
    const { mujid, menteePersonalDetails } = requestBody;
    ...
    const mentor = await Mentor.findOne({ mujid: requestBody.mentorMujid });
    if (!mentor) {
...
export async function PUT(req) {
  try {
    ...
    return NextResponse.json(
      { message: "Mentee updated successfully", updatedMentee },
      { status: 200 }
    );
  } catch (error) {
...
export async function PATCH(req) {
  try {
    ...
    // Use partial validation for PATCH requests
    const { error } = Joi.object(updateData)
      .fork(Object.keys(updateData), (schema) => schema.optional())
      .validate(updateData);
    if (error) {
      return createErrorResponse(error.details[0].message, 400);
    }
    ...
    console.error("Error partially updating mentee:", error);
    return createErrorResponse("Error updating mentee", 500);
  }
}
