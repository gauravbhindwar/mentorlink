...
const mentorSchema = Joi.object({
  ...
  roles: Joi.array().items(Joi.string().valid('mentor', 'admin', 'superadmin')).default(['mentor']),
});
...
export async function POST(req) {
  try {
    ...
    const { email, name, mujid, phone, designation, roles } = requestBody;
...
export async function PATCH(req) {
  try {
    ...
    const { mujid, email, name, phone, designation, roles } = requestBody;
    ...
    if (roles) existingMentor.roles = roles;
