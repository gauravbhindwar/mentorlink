...
const ConsolidatedReport = () => {
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      const updatedParsedMeetingdata = parsedMeetingdata.map((meetingEntry) => {
        const updatedMenteeDetails = meetingEntry.menteeDetails.map(
          (mentee) => {
      ...
      sessionStorage.setItem(
        "meetingData",
        JSON.stringify(updatedParsedMeetingdata)
      );
  ...
  const isRemarkModified = (MUJid) => {
    return (
      pendingChanges[MUJid] !== undefined &&
    ...
    switch (e.key) {
      ...
      case " ":
        ...
        if (type === "row") {
          handleRemarksClick(
            mentees[index].MUJid,
            pendingChanges[mentees[index].MUJid] || mentees[index].mentorRemarks
          );
        } else if (type === "save") {
          saveAllChanges();
    ...
    return (
      <tr
        className={`hover:bg-gray-700/30 transition-colors ${
          currentFocus === index
            ? "bg-gray-700/50 ring-2 ring-orange-500/50"
            : ""
        }`}>
        ...
        <td className='px-6 py-4 text-center'>
          <div className='flex flex-col gap-1'>
            ...
            <span className='text-xs text-gray-400'>
              {Math.round(
                (mentee.meetingsCount / mentee.meetingsTotal) * 100
              ) || 0}
              % attendance
            </span>
          </div>
  ...
  return (
    <div className='min-h-screen bg-gray-900 pt-16'>
      <div className='h-[calc(100vh-4rem)] max-w-[85rem] mx-auto p-8 flex flex-col overflow-hidden'>
        {" "}
        ...
        <div className='flex gap-6 flex-1 min-h-0'>
          ...
          <div className='flex-1 max-w-[calc(100%-16rem)] flex flex-col min-h-0'>
            {" "}
            {/* Added min-h-0 */}
              ...
              <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4'>
                <div className='space-y-1'>
                  ...
                  <p className='text-gray-400'>
                    View and manage all mentee reports
                  </p>
                </div>
            ...
            <div className='flex-1 bg-gray-800 rounded-xl shadow-xl overflow-hidden flex flex-col min-h-0'>
              {" "}
              {/* Added min-h-0 */}
              {/* Fixed Header */}
          ...
          <div className='w-60 space-y-4 flex-shrink-0 overflow-y-auto'>
            ...
            {hasChanges ? (
              ...
              // Show PDF Download button or warning based on meeting count
              (() => {
                const semesterMeetings = meetings.filter(
                  (meeting) => meeting && meeting.semester === selectedSemester
                );
        ...
        <RemarksDialog
          ...
          originalValue={
            dialogState.currentMUJid
              ? originalRemarks[dialogState.currentMUJid]
              : ""
          }
