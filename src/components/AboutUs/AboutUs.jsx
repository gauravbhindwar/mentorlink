...
const AboutUs = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] overflow-hidden relative pt-16">
      ...
      <div className={`relative z-10 container mx-auto px-4 ${quicksand.className}`}>
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-center py-8"
        >
          <h1 className={`${righteous.className} text-5xl md:text-7xl font-normal bg-clip-text text-transparent bg-gradient-to-r from-orange-400 to-blue-500 mb-8 tracking-wide`}>
            About MentorLink
          </h1>
          <p className="text-gray-300 max-w-2xl mx-auto mb-12 text-xl font-light leading-relaxed">
            A project developed under the Software Development Club (SDC), Department of Computer Science Engineering, Manipal University Jaipur.
          </p>
        </motion.div>
        {/* Faculty Section */}
        <h2 className={`${righteous.className} text-3xl font-normal text-blue-400 text-center mb-8 tracking-wide`}>
          Faculty Guides
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {facultyMembers.map((faculty) => (
            ...
            >
              ...
              <h3 className="text-lg font-semibold text-white text-center mb-1">{faculty.name}</h3>
              <p className="text-blue-400 text-center text-sm mb-1">{faculty.role}</p>
              <p className="text-gray-400 text-center text-sm mb-2">{faculty.description}</p>
              <div className="flex flex-col items-center gap-2 text-sm">
                <a href={`mailto:${faculty.email}`} className="text-gray-400 hover:text-blue-400 flex items-center gap-2">
                  <FaEnvelope size={14} /> {faculty.email}
                </a>
                <a href={`tel:${faculty.phone}`} className="text-gray-400 hover:text-blue-400 flex items-center gap-2">
                  <FaPhone size={14} /> {faculty.phone}
                </a>
                <a href={faculty.linkedin} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 mt-2">
                  <FaLinkedin size={20} />
                </a>
              </div>
            </motion.div>
        ...
        {/* Developers Section */}
        <h2 className={`${righteous.className} text-3xl font-normal text-orange-400 text-center mb-8 tracking-wide`}>
          Development Team
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {teamMembers.map((member) => (
            ...
            >
              ...
              <p className="text-orange-400 text-center text-sm mb-1">{member.role}</p>
              <p className="text-gray-400 text-center text-sm mb-2">{member.description}</p>
              <div className="flex flex-col items-center gap-2 text-sm">
                <motion.a 
        ...
        <footer className="text-center py-6 border-t border-gray-800">
          <p className="text-gray-400 text-sm tracking-wide font-light">
            © {new Date().getFullYear()} MentorLink - Department of Computer Science Engineering, Manipal University Jaipur.
            <br />
            All rights reserved.
          </p>
        </footer>
