...
export default function NavbarNew() {
    ...
    useEffect(() => {
        ...
        const storedMentorData = sessionStorage.getItem("mentorData");

        if (storedEmail) setEmail(storedEmail);
        if (storedRole) setRole(storedRole);
        if (storedMentorData) setMentorData(storedMentorData);
    }, []);
    ...
    const adminButtons = [
        {
            name: "Manage Mentees",
            url: "/pages/admin/managementee",
        },
        {
            name: "Manage Mentors",
            url: "/pages/admin/managementor",
        },
        {
            name: "Manage Meetings",
            url: "/pages/admin/managemeeting",
        },
        {
            name: "Create Academic Session",
            url: "/pages/admin/createacademicsession",
        },
        {
            name: "Archive",
            url: "/archives",
    ...
    const mentorButtons = [
        ...
        {
            name: "Consolidated Meeting Report",
            url: "/pages/mentordashboard/consolidatedReport",
        },
    ];

    return (
        <nav className="h-16 z-50 fixed top-0 bg-[#181322] text-white font-medium text-lg w-screen flex justify-between items-center px-4">
            ...
            <div className="mid-col hidden md:flex justify-around w-1/2">
                {role === "admin" && (
                    <div className=" group relative">
                        <button
                            className="cursor-pointer group-hover:scale-110 hover:underline transition-all duration-100"
                            onClick={(e) => {
                                router.push("/pages/admin/admindashboard");
                            }}
                        >
                        ...
                        <div className="opacity-0 invisible group-hover:visible group-hover:opacity-100 flex flex-col items-center justify-around py-8 transition-all duration-100 absolute top-8 rounded-xl -left-20 min-h-fit gap-4 font-normal w-80 bg-gradient-to-b from-[#181322] to-slate-800">
                            {adminButtons.map((btn) => {
                                return (
                                    <button
                                        key={btn.url}
                                        className="hover:underline"
                                        onClick={(e) => router.push(btn.url)}
                                    >
                                        {btn.name}
                                    </button>
                                );
            ...
            <div className="right-col flex gap-6">
                <Image
                    src={"/sdc-logo.webp"}
                    width={1000}
                    height={1000}
                ...
                <div className="relative group">
                    <button className="h-10 md:h-12 w-10 md:w-12 border-2 border-amber-600 rounded-full text-[1.5rem]">
                        👤
                    </button>
                    <div className="opacity-0 invisible group-hover:visible group-hover:opacity-100 flex flex-col items-center justify-around py-8 transition-all duration-100 absolute top-12 md:top-14 rounded-bl-xl -right-4 min-h-fit gap-4 font-normal w-80 bg-gradient-to-b from-[#181322] to-slate-800 ">

                        {email && <div className="underline underline-offset-[6px]">{email}</div>}
                        <button onClick={e => router.push("/about")} className="hover:underline">About Us</button>
                        {role === "admin" &&
                            <button onClick={(e) => router.push('/pages/admin/admindashboard')} className="hover:underline">Admin Dashboard</button>
                        }
                        {(role === "admin" || role === "mentor") &&
