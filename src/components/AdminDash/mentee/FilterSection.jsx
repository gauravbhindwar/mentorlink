...
const FilterSection = ({
  ...
  }, [filters.academicYear, handleFilterChange]);
  ...
  const handleBulkAddMentees = async (menteesData) => {
    try {
      ...
      if (response.status === 201) {
        showAlert(
          `Successfully added ${response.data.savedCount} mentees with ${response.data.relationshipsCreated} mentor relationships`,
          "success"
        );
        handleSearch();
      } else {
  ...
  useEffect(() => {
    if (!filters.academicYear || !filters.academicSession) {
      ...
      const currentSession =
        currentMonth >= 7 && currentMonth <= 12 ? sessions[0] : sessions[1];
  ...
  const renderYearDropdown = () =>
    renderDropdown(
      ...
      <Box
        ...
        sx={{ position: "fixed", transform: "translateY(100%)" }}>
        {(yearSuggestions.length > 0
          ...
          : (() => {
              ...
              return [0, 1, 2, 3].map(
                (offset) =>
                  `${currentYear - offset}-${currentYear - offset + 1}`
              );
            })()
        ).map((year) => (
          <Box
