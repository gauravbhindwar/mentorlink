...
const ManageAcademicSession = () => {
  ...
  const [existingSessions, setExistingSessions] = useState<
    AcademicSessionType[]
  >([]);
  const [upcomingSession, setUpcomingSession] = useState<SessionInfo | null>(
    null
  );
  ...
  const handleCreateAcademicSession = async () => {
    ...
    try {
      ...
      const [startYear, endYear] = sessionToCreate.academicYear
        .split("-")
        .map(Number);
      const sessionData = {
      ...
      if (response.status === 200) {
        ...
        const { currentSessionInfo, upcomingSessionInfo } =
          determineUpcomingSession();
        ...
        const currentExists = updatedSessions.data.some(
          (session: AcademicSessionType) =>
            session.sessions.some(
              (s) => s.name === currentSessionInfo.sessionName
            )
        );
        const upcomingExists = updatedSessions.data.some(
          (session: AcademicSessionType) =>
            session.sessions.some(
              (s) => s.name === upcomingSessionInfo.sessionName
            )
        );
      ...
      if (axios.isAxiosError(error) && error.response) {
        showToast(
          error.response.data?.error || "Error creating academic session",
          "error"
        );
      } else {
  ...
  const handleArchiveSession = async (startYear: number, endYear: number) => {
    setAlertConfig({
      ...
      message:
        "Are you sure you want to archive this academic session?\nThis action cannot be undone.",
      onConfirm: async () => {
        try {
          setLoading(true, 0, "Starting archive process...");

          const response = await axios.put(
            "/api/admin/academicSession/archive",
            {
              start_year: startYear,
              end_year: endYear,
            }
          ...
          if (response.status === 200) {
            ...
            const updatedSessions = await axios.get(
              "/api/admin/academicSession"
            );
            setExistingSessions(updatedSessions.data);
          ...
          if (axios.isAxiosError(error) && error.response) {
            showToast(
              `Error: ${
                error.response.data?.error || "Failed to archive session"
              }`,
              "error"
            );
          } else {
  ...
  const handleChangeCurrentSession = async () => {
    ...
    const upcomingSession = existingSessions.find(
      (s) =>
    ...
    setAlertConfig({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const response = await axios.put(
            "/api/admin/academicSession/changeToUpcoming",
            {
              currentSession: {
          ...
          if (axios.isAxiosError(error) && error.response) {
            showToast(
              error.response.data?.error || "Error changing session",
              "error"
            );
          } else {
  ...
  const renderStatusChip = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium ${config.bg} ${config.text} border ${config.border} backdrop-blur-sm flex items-center gap-1.5 shadow-sm`}
      >
        {config.icon}
        {config.label}
      </span>
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        ...
        const currentExists = response.data.some(
          (session: AcademicSessionType) =>
            session.sessions.some(
              (s) => s.name === currentSessionInfo.sessionName
            )
        ...
        const upcomingExists = response.data.some(
          (session: AcademicSessionType) =>
            session.sessions.some(
              (s) => s.name === upcomingSessionInfo.sessionName
            )
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-4">
          ...
          <div className="lg:col-span-5 space-y-3 sm:space-y-4">
            <motion.div
              ...
              className="bg-black/40 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/5 shadow-xl h-auto sm:h-auto"
            >
              ...
              {!isSessionExists ? (
                ...
                <div className="h-full flex flex-col items-center justify-center px-3 sm:px-4 py-8 sm:py-12">
                  <div className="w-40 h-40 sm:w-56 sm:h-56 mb-6 sm:mb-8">
                    {" "}
                    {/* Increased size from w-48 h-48 */}
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-400 max-w-md">
                      {" "}
                      {/* Increased max-width */}
                      Your academic calendar is fully synchronized and up to
                      date. The system is actively monitoring session timelines
                      and will notify you when the next academic period needs to
                      be created.
                    </p>
