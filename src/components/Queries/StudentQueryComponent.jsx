      ...
      >
        ...
        <div className="relative z-10 container mx-auto px-4 pt-20">
          ...
          >
            ...
            <p className="text-gray-300 mt-2">Handle student support requests and inquiries</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {['Pending', 'Resolved', 'Open'].map((status, index) => (
              ...
              >
                <h3 className="text-xl font-semibold text-white">{status} Queries</h3>
                <p className="text-gray-400 mt-2">View all {status.toLowerCase()} queries</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        {selectedStatus && (
          ...
          >
            <div className="bg-[#1a1a1a] p-6 rounded-lg max-w-3xl w-full">
              <h2 className="text-2xl font-bold text-white mb-4">{selectedStatus} Queries</h2>
              <div className="space-y-4">
                {queries.filter(query => query.status === selectedStatus).map(query => (
                  <div 
                    key={query.id}
                    className="bg-white/10 rounded-xl p-4 border border-white/10 cursor-pointer"
                    onClick={() => handleQueryClick(query)}
                  >
                    <h3 className="text-xl font-semibold text-white">{query.student}</h3>
                    <p className="text-gray-400">{query.subject}</p>
                  </div>
                ))}
              </div>
              <button className="mt-4 btn-orange" onClick={closePopup}>Close</button>
            </div>
          </motion.div>
        )}
        ...
        {selectedQuery && (
          ...
          >
            <div className="bg-[#1a1a1a] p-6 rounded-lg max-w-3xl w-full">
              <h2 className="text-2xl font-bold text-white mb-4">Query Details</h2>
              <div className="space-y-2">
                <p className="text-white"><strong>Student:</strong> {selectedQuery.student}</p>
                <p className="text-white"><strong>Email:</strong> {selectedQuery.email}</p>
                <p className="text-white"><strong>Section:</strong> {selectedQuery.section}</p>
                <p className="text-white"><strong>MUJ ID:</strong> {selectedQuery.mujId}</p>
                <p className="text-white"><strong>Department:</strong> {selectedQuery.department}</p>
                <p className="text-white"><strong>Phone Number:</strong> {selectedQuery.phoneNumber}</p>
                <p className="text-white"><strong>Description:</strong> {selectedQuery.description}</p>
              </div>
              <div className="mt-4">
                <textarea
